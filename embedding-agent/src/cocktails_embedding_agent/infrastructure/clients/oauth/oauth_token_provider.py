...
class OAuthTokenProvider(IOAuthTokenProvider):
    ...
    @inject
    def __init__(self, oauth_options: OAuthOptions):
    ...
    async def _fetch_token(self) -> str:
        ...
        Raises:
            RuntimeError: If token acquisition fails.
        """
        token_url = f"https://{self.oauth_options.domain}/oauth/token"

        try:
            async with AsyncOAuth2Client(
                client_id=self.oauth_options.client_id,
                client_secret=self.oauth_options.client_secret,
            ) as client:
                token = await client.fetch_token(
                    token_url,
                    grant_type="client_credentials",
                    audience=self.oauth_options.audience,
                    scope=self.oauth_options.scope,
                )
